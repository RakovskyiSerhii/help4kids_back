...
Future<Response> onRequest(RequestContext context) async {
  final handler = requireAdmin((context) async {
    ...
    try {
        ...
        if (part['name'] == 'image' || part['name'] == 'file') {
          ...
          if (fileData != null && fileData.isNotEmpty) {
            ...
            if (!contentType.startsWith('image/')) {
              return ResponseHelpers.error(
                ApiErrors.validationError('File must be an image'),
              );
            }
            ...
            if (fileData.length > 5 * 1024 * 1024) {
              return ResponseHelpers.error(
                ApiErrors.validationError('Image size must not exceed 5MB'),
              );
            }
            
            filename = part['filename'] as String? ?? 'image';
            final ext = filename.split('.').last.toLowerCase();
            if (!['jpg', 'jpeg', 'png', 'gif', 'webp'].contains(ext)) {
              return ResponseHelpers.error(
                ApiErrors.validationError('Image must be jpg, jpeg, png, gif, or webp'),
              );
            }
            ...
            final uploadsDir = Directory('uploads/images');
      ...
      if (imageFile == null) {
        return ResponseHelpers.error(
          ApiErrors.validationError('No image file provided'),
        );
      }
      
      // Return the file URL (in production, this would be a CDN URL)
      final fileUrl = '/uploads/images/${imageFile.path.split('/').last}';
      ...
      return ResponseHelpers.error(
        ApiErrors.internalError('Failed to upload image: ${e.toString()}'),
      );
    }
  });
